	...
	public class PromotionService : IPromotionService
	{
		...
		public async Task<IDataResult<List<PromotionListDTO>>> GetAllAsync(string sortOrder, string searchQuery)
		{
			try
			{
                ...
                {
                    // "%" işaretini temizleyelim ki hem "%20" hem "20" aramasında çalışsın
                    var cleanedSearch = searchQuery.Replace("%", "").Trim();

                    promotionListDTOs = promotionListDTOs
                        .Where(p =>
                            // Şirket adı
                            companies.Any(c => c.Id == p.CompanyId &&
                                c.Name.Contains(searchQuery, StringComparison.OrdinalIgnoreCase)) ||

                            // Ürün adı
                            products.Any(prod => prod.Id == p.ProductId &&
                                prod.Name.Contains(searchQuery, StringComparison.OrdinalIgnoreCase)) ||

                            // İndirim oranı (örn. %20 veya 20)
                            p.Discount.ToString().Contains(cleanedSearch, StringComparison.OrdinalIgnoreCase)
                        )
                        .ToList();
                }
        ...
        /// <summary>
        /// Şirket id bilgisi ile şirkete ait promosyonları getirir.
        /// </summary>
        /// <param name="companyId"></param>
        /// <returns> Asenkron işlemi temsil eden bir görev. Görev sonucunda promosyon listesini döndürür.</returns>
        public async Task<List<Promotion>> GetPromotionsListByCompanyIdAsync(Guid companyId)
		{
            var promotions = await _promotionRepository.GetAllAsync();

            return promotions
				.Where(promotion => promotion.CompanyId == companyId)
				.ToList();
        }
    }
}
