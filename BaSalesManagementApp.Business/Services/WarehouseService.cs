...
namespace BaSalesManagementApp.Business.Services
{
    ...
    public class WarehouseService : IWarehouseService
    {
        ...
        public async Task<List<Warehouse>> GetWarehousesByBranchIdAsync(List<Branch> branches)
        {
            var warehouses = await _warehouseRepository.GetAllAsync();

            var branchsIds = branches.Select(b => b.Id).ToList();

            return warehouses
                   .Where(warehouse => branchsIds.Contains(warehouse.BranchId))
                   .ToList();
        }

        //ŞİRKETİN ŞUBELERİNE GÖRE DEPOLARINA DÖNER
        public async Task<IDataResult<List<WarehouseDTO>>> GetWarehousesByBranchIdAsynca(List<Branch> branches)
        {
            try
            {
                var warehouses = await _warehouseRepository.GetAllAsync();
                ...
                var filteredWarehouses = warehouses
                    .Where(warehouse => branchIds.Contains(warehouse.BranchId))
                    .ToList();

                // Warehouse listesini WarehouseDTO'ya dönüştür
                var warehouseDTOs = filteredWarehouses.Adapt<List<WarehouseDTO>>();

                return new SuccessDataResult<List<WarehouseDTO>>(warehouseDTOs, "Warehouse'lar başarıyla getirildi.");
            }
            catch (Exception)
            {
                return new ErrorDataResult<List<WarehouseDTO>>("Warehouse'lar getirilirken bir hata oluştu.");
            }
        }

        public async Task<IDataResult<List<WarehouseListDTO>>> GetAllAsyncByCompanyId(Guid? companyId)
        {
            try
            {
                ...
                var warehouses = await _warehouseRepository.GetAllAsync();
                var branchList =  await _branchRepository.GetBranchesByCompanyIdAsync(companyId);

                var filteredWarehouses = await GetWarehousesByBranchIdAsync(branchList);

                if (!filteredWarehouses.Any())
                {
                    return new ErrorDataResult<List<WarehouseListDTO>>(new List<WarehouseListDTO>(), "Bu şirkete ait depo bulunamadı.");
                }

                var warehousesListDTOs = filteredWarehouses.Adapt<List<WarehouseListDTO>>();
                return new SuccessDataResult<List<WarehouseListDTO>>(warehousesListDTOs, Messages.Warehouse_LISTED_SUCCESS);
            }
            catch (Exception ex)
            {
                return new ErrorDataResult<List<WarehouseListDTO>>(new List<WarehouseListDTO>(), Messages.Warehouse_LIST_FAILED + " " + ex.Message);
            }
        }
