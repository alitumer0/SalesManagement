...
namespace BaSalesManagementApp.Business.Services
{
    public class BranchService : IBranchService
    {
        ...
        public async Task<IDataResult<List<BranchListDTO>>> GetBranchesByCompanyIdAsynca(Guid companyId)
        {
            try
            {
                var branches = await _branchRepository.GetBranchesByCompanyIdAsync(companyId);

                // Status değeri 3 olmayanları filtrele
                var filteredBranches = branches.Where(branch => branch.Status != (Status)3).ToList();

                if (!filteredBranches.Any())
                {
                    return new ErrorDataResult<List<BranchListDTO>>(new List<BranchListDTO>(), "Bu şirkete ait uygun şube bulunamadı.");
                }

                var branchListDTOs = filteredBranches.Adapt<List<BranchListDTO>>();
                return new SuccessDataResult<List<BranchListDTO>>(branchListDTOs, "Şubeler başarıyla getirildi.");
            }
            catch (Exception ex)
            {
                return new ErrorDataResult<List<BranchListDTO>>(null, $"Hata oluştu: {ex.Message}");
            }
        }

        public async Task<bool> HasWarehouseAsync(Guid branchId)
        {
            var branch = await _branchRepository.GetByIdAsync(branchId);
            return branch?.Warehouses != null; // Eğer Warehouse null değilse, şubenin deposu var demektir.
        }
